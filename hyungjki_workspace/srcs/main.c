...
static char	*ft_substr1(char *s, int start, int len)
{
	char	*ret;
	int	ed;
	int	st;
...
static char	**ft_split1(char *s, char c)
{
	char	**ret;
	int		ret_st;
	int		st;
	...
	{
		if (!(ret[ret_st++] = ft_substr1(s, st, ft_get_next_idx(s, c, st) - st)))
		{
...
int *get_meta_arr(char *str)
{
	...
	ret = (int *)malloc(sizeof(int) * ft_strlen1(str));
	st = 0;
	while (str[st])
	{
		if (str[st] == '\'')
...
int quotes_sub_loop(char **buf, int **meta_arr, char target)
{
	...
	while (cnt_quotes(*buf, target, *meta_arr) % 2 == 1)
	{
		...
		ft_resize_and_copy(buf, temp, 0, ft_strlen1(temp));
		free(temp);
		free(*meta_arr);
...
static int	ft_strncmp1(char *s1, char *s2, int len)
{
	int	st;
...
static char	*ft_strnstr1(char *big, char *little, size_t len)
{
	size_t	st;
	size_t	lt_len;

	st = 0;
	lt_len = ft_strlen1(little);
	if (little[0] == '\0')
		return ((char *)big);
	else if (!big)
		if (big[st])
	...
	while (st + lt_len <= len)
	{
		...
		if (!big[st] || st + lt_len > len)
			break ;
		if (!ft_strncmp1(&big[st], little, lt_len))
			return (&((char *)big)[st]);
		st++;
	}
	return (0);
...
t_string *ft_lstfind(t_string *root, char *target)
{
	...
	while (curr)
	{
		if (ft_strnstr1(curr->str, target, ft_strlen1(curr->str)) != 0)
			return (curr);
		curr = curr->next;
	}
	return (0);
...
static void ft_lstfree1(t_string *del)
{
	free(del->str);
	free(del);
	del = 0;
...
t_string *ft_lstremove(t_string **root, t_string *target)
{
	...
	curr = *root;
	prev = 0;
	while (curr && ft_strncmp1(curr->str, target->str, ft_strlen1(curr->str)) != 0)
	{
		prev = curr;
		curr = curr->next;
	}
...
void ft_lstfree1_all(t_string *root)
{
	t_string *del;
...
void ft_inst_free(t_inst *root)
{
	...
	while (del)
	{
		...
		free(del->inst);
		free(del->option);
		ft_lstfree1_all(del->rd);
		ft_lstfree1_all(del->arg);
...
static void ft_lstadd_front1(t_string **root, t_string *s)
{
	t_string *curr;
...
static void ft_lstadd_back1(t_string **root, t_string *s)
{
	t_string *curr;
...
char *replace_str(char **buf, int st, int ed, char *to)
{
	...
	if (st == ed - 1)
		return (0);
	now_str = ft_substr1(*buf, st, ed - st + 1);
	ft_resize_and_copy(&temp, *buf, 0, st);
	ft_resize_and_copy(&temp, to, 0, ft_strlen1(to));
	ft_resize_and_copy(&temp, *buf, ed + 1, ft_strlen1(*buf));
	free(*buf);
	*buf = temp;
	return (now_str);
}
...
char **split_redirection(char *str, char **splitter)
{
	...
	{
		ret = ft_split1(str, '>');
		if (ft_strnstr1(str, ">>", ft_strlen1(str)) != 0)
			*splitter = ">>";
		else
			*splitter = ">";
	}
	else if (ft_strchr1(str, '<') != 0)
	{
		ret = ft_split1(str, '<');
		*splitter = "<";
	}
...
int check_red_error(char *inst)
{
	...
	if (num_of_lines > 2 || num_of_lines2 > 2)
		return (1);
	if (ft_strchr1(inst, '<') && ft_strchr1(inst, '>'))
		return (1);
	return (0);
}
		...
		if (ft_cnt_lines(now, red[0]) == 2)
		{
			if (is_fd(chunks[0]) == 0)
			{
				...
				if (chunks[0][0] == '1' && red[0] == '>')
					...
					ft_lstadd_back1(&(inst->rd), ft_lstinit(ft_strdup(chunks[0])));
			}
			else if (is_fd(chunks[0]) == 1)
			{
				if (inst->inst == 0)
					...
					ft_lstadd_back1(&(inst->arg), ft_lstinit(ft_strdup(chunks[0])));
			}
			else
			{
			...
			ft_lstadd_back1(&(inst->rd), ft_lstinit(ft_strdup(red)));
			ft_lstadd_back1(&(inst->rd), ft_lstinit(ft_strdup(chunks[1])));
		}
		else if (ft_cnt_lines(now, red[0]) == 1)
		{
			ft_lstadd_back1(&(inst->rd), ft_lstinit(ft_strdup(chunks[0])));
			if (ft_strncmp1(now, red, ft_strlen1(red)) == 0)
				ft_lstadd_front1(&(inst->rd), ft_lstinit(ft_strdup(red)));
			else
			{
				ft_lstadd_back1(&(inst->rd), ft_lstinit(ft_strdup(red)));
				if (*(cmd + *k + 1) == 0)
				...
				ft_lstadd_back1(&(inst->rd), ft_lstinit(ft_strdup(*(cmd + ++(*k)))));
				}
		}
		else
		{
...
t_inst *make_command(char **space_chunks, int line_cnt)
{
	...
	while (*(space_chunks + k))
	{
		...
		else if (flag == -1)
		{
			...
			else if (ft_strncmp1(*(space_chunks + k), "-", 1) == 0)
			{
				...
				else if (ret->arg != 0)
				{
					...
					ft_lstadd_back1(&ret->arg, arg);
				}
			}
			else
			{
				arg = ft_lstinit(ft_strdup(*(space_chunks + k)));
				if (ft_strncmp1(arg->str, "\"\"", ft_strlen1(arg->str)) == 0)
					ft_lstfree1(arg);
				else if (ft_strncmp1(arg->str, "\'\'", ft_strlen1(arg->str)) == 0)
					ft_lstfree1(arg);
				else
					ft_lstadd_back1(&ret->arg, arg);
			}
...
void echo_merge_args(t_inst **inst)
{
	...
	while (arg)
	{
		ft_resize_and_copy(&temp, arg->str, 0, ft_strlen1(arg->str));
		if (arg->next)
			ft_resize_and_copy(&temp, " ", 0, 1);
		arg = arg->next;
	}
	ft_lstfree1_all((*inst)->arg);
...
t_inst *split_commands(char **semi_chunks, int line_cnt)
{
	...
	while (*(semi_chunks + i))
	{
		pipe_chunks = ft_split1(*(semi_chunks + i), '|');
		j = 0;
		temp = 0;
		while (*(pipe_chunks + j))
		{
			inst = make_command(ft_split1(*(pipe_chunks + j), ' '), ft_cnt_lines(*(pipe_chunks + j), ' '));
			if (inst == 0)
				printf("err 발생..\n");
			else
			{
				if (ft_strncmp1(inst->inst, "echo", ft_strlen1(inst->inst)) == 0)
...
char *decap_quotes(char *str, char *quote)
{
	...
	ret = ft_substr1(str, 1, ft_strlen1(str) - 2);
	*quote = str[0];
	return (ret);
}
...
void handle_quotes(char **buf, t_string **const_strings)
{
	...
	while ((now = *(*buf + st)))
	{
		if ((now == '\'' || now == '\"') && meta_arr[st] == 1)
		{
			...
			{
				ft_lstadd_back1(const_strings, ft_lstinit(ft_strdup(prev_str)));
				free(meta_arr);
				meta_arr = get_meta_arr(*buf);
			}
			free(rep);
...
t_env *ft_envfind(char *key)
{
	...
	if (curr == 0)
		return (0);
	while (curr && ft_strncmp1(curr->key, key, ft_strlen1(curr->key)) != 0)
	{
		curr = curr->next;
	}
	return (curr);
...
void ft_envremove(t_env **root, char *key)
{
	...
	while (curr)
	{
		if (ft_strncmp1(curr->key, key, ft_strlen1(curr->key)) == 0)
		{
			if (prev == 0)
				*root = curr->next;
			else
...
void ft_envchkandadd(t_env **root, t_env *now, int flag)
{
	...
	{
		if (flag == 1)
			ft_resize_and_copy(&curr->value, now->value, 0, ft_strlen1(now->value));
		else
		{
			free(curr->value);
	...
	while (s[st])
	{
		...
		else if (s[st] == '$' && meta_arr[st] == 1)
		{
			...
			while (s[ed] && ((s[ed] >= 'A' && s[ed] <= 'Z') ||
						(s[ed] >= '0' && s[ed] <= '9')))
				ed++;
			env = ft_envfind(ft_substr1(s, st + 1, ed));
			if (env != 0)
				ft_resize_and_copy(&ret, env->value, 0, ft_strlen1(env->value));
			st = ed - 1;
		}
		else
		...
		while (arg)
		{
			...
			while ((arg->str)[st])
			{
				if (((arg->str)[st] == '\'' || (arg->str)[st] == '\"')
						&& (ft_strncmp1(arg->str + st, encap_quotes("%%", (arg->str)[st]),
								4) == 0))
				{
					ft_resize_and_copy(&temp, const_now->str, 0, ft_strlen1(const_now->str));
					st += 3;
...
int main(int argc, char **argv, char **envp){
	...
	while (1)
	{
		...
		while (curr != 0)
		{
			...
			// cd
			if (ft_strnstr1(curr->inst, "cd", ft_strlen1(curr->inst)) != 0
					&& ft_strlen1(curr->inst) == 2)
			{
				if (ft_cd(curr) == 0)
				{
					free(prompt);
			...
			else if (ft_strnstr1(curr->inst, "pwd", ft_strlen1(curr->inst)) != 0
					&& ft_strlen1(curr->inst) == 3)
			{
				if (ft_pwd() == -1)
					printf("error 발생\n");
			}
			else if (ft_strnstr1(curr->inst, "ls", ft_strlen1(curr->inst)) != 0
					&& ft_strlen1(curr->inst) == 2)
			{
				if (ft_ls(curr) == 1)
				{
			...
			else if (ft_strnstr1(curr->inst, "exit", ft_strlen1(curr->inst)) != 0
					&& ft_strlen1(curr->inst) == 4)
			{
				free_genv();
				free(prompt);
				ft_lstfree1_all(const_strings);
				ft_inst_free(insts);
				exit(0);
			}
			else if (ft_strnstr1(curr->inst, "export", ft_strlen1(curr->inst)) != 0
					&& ft_strlen1(curr->inst) == 6)
			{
				int temp_ret;
			...
			else if (ft_strnstr1(curr->inst, "env", ft_strlen1(curr->inst)) != 0
					&& ft_strlen1(curr->inst) == 3)
			{
				if ((ft_env(curr)) != 0)
					printf("err?\n");
			}
			else if (ft_strnstr1(curr->inst, "unset", ft_strlen1(curr->inst)) != 0
					&& ft_strlen1(curr->inst) == 5)
			{
				if ((ft_unset(curr)) != 0)
					printf("err?\n");
			}
			else if (ft_strnstr1(curr->inst, "echo", ft_strlen1(curr->inst)) != 0
					&& ft_strlen1(curr->inst) == 4)
			{
				if ((ft_echo(curr)) != 0)
					printf("echo err?\n");
